...
class TrendViewController: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCellWithIdentifier("TrendCell", forIndexPath:indexPath)
        var count:Int
        ...
        cell.textLabel?.text = self.results[indexPath.row]["name"].string
    ...
    func connectionDidFinishLoading(connection: NSURLConnection!) {
       ...
       // let jsonResults = (try! NSJSONSerialization.JSONObjectWithData(self.buffer, options: [])) as! NSDictionary
        ...
        self.results = json[0]["trends"]
        // Save statuses for table use
       // self.results = json["trends"]
