...
class LocationSearchTable: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.textL?.text = self.results[indexPath.row]["text"].string
        cell.nameT?.text = self.results[indexPath.row]["user"]["name"].string
       ...
       // cell.nameT?.text = tweet["user"]?.valueForKey("name") as? String
        ...
        var im = self.results[indexPath.row]["user"]["profile_image_url"].string
        let imageURL:NSURL? =  NSURL(string: im!)
        ...
        self.accountStore.requestAccessToAccountsWithType(accountType, options: nil) {
            ...
            if granted {
                ...
                if twitterAccounts?.count == 0
                {
                    ...
                    print(self.latitude + "," + self.longitude + "," + self.radius + "mi")
                    ...
                    let param = [
                        "q": self.searchTerm,
                        "result_type": "recent",
                        "geocode": self.latitude + "," + self.longitude + "," + self.radius + "mi"
    ...
    func connectionDidFinishLoading(connection: NSURLConnection!) {
     ...
     //   self.results = jsonResults["statuses"] as! NSMutableArray
        let json = JSON(data: self.buffer)
